    (function ( $ ) {
        ...
        $.fn.matrixchart = function(options) {
            ...
            for (var i=1;i<=settings.noOfxLevels; i++){
                var ImageLink = '<img src="' + settings.xlabels[i - 1].roe_image + '">';

                xlevelColor =  HSVtoRGB; //(initialStarth + ((i-1) * xcolordivision),HSVsValue,HSVvValue);
                xlevelpoints += '<div class="axis-point-level level-' + i + '" style="width:' + xcellsize + '%"><span style="background: rgb(' + xlevelColor[0] + ',' + xlevelColor[1] + ',' + xlevelColor[2] + ')">' + ImageLink + '</span><p>' + settings.xlabels[i - 1].roe_name+'</p></div>';
            }

            // for-ylevels
            var ylevelpoints = "";
            for (var i=1;i<=settings.noOfyLevels; i++){
                var ImageLink = '<img src="' + settings.ylabels[i - 1].roe_image + '">';

                ylevelColor =  HSVtoRGB; //(initialStarth + ((i-1) * ycolordivision),HSVsValue,HSVvValue);
                ylevelpoints += '<div class="axis-point-level level-' + i + '" style="height:' + ycellsize + '%"><span style="background: rgb(' + ylevelColor[0] + ',' + ylevelColor[1] + ',' + ylevelColor[2] + ')">' + ImageLink + '</span><p>' + settings.ylabels[i - 1].roe_name+'</p></div>';
            }
            ...
            for (var xaxis=1;xaxis<=settings.noOfxLevels; xaxis++){
                for(var yaxis=1;yaxis<=settings.noOfyLevels; yaxis++){
                    matrixmark+= '<div class="value-pocket value-'+xaxis+'-'+yaxis+'" style="height:'+settings.valuepocketHeight+';width:'+settings.valuepocketwidth+'px;left:calc('+xcellsize+'% * '+xaxis+' - '+settings.valuepocketHeight+'px);bottom:calc('+ycellsize+'% * '+ yaxis+' - '+settings.valuepocketHeight/2+'px - '+ycellsize+'%)"><ul></ul></div>';
                }
            }
            ...
            $.each(settings.matrixDatas, function(index, value){
                ...
                if(children < 2){
                    element.append('<li><div class="popup" onclick="myFunction()"><img src="'+value.image+'"/></div></li>');
                }else if(children == 2){
